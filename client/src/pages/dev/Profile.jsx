...
import CertificateEdit from "../../components/profileEditComponentts/CertificateEdit";

const Profile = ({ dev }) => {
  const { user } = useAuthStore();
  const [showEditImg, setShowEditImg] = useState(false);
  const [showEditProfile, setShowEditProfile] = useState(false);
  const [showEditCertificates, setShowEditCertificates] = useState(false);
  ...
  return (
    <div className="flex flex-col justify-between h-full gap-6 px-6 pb-6">
      ...
      {showEditCertificates && (
        <CertificateEdit setShowEditCertificates={setShowEditCertificates} />
      )}
      <AddProject />
      ...
      <div className="h-full overflow-y-scroll no-scrollbar flex flex-col gap-6">
        <div className=" mx-auto rounded-lg  grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="col-span-1 ">
            ...
            <div className="bg-white p-4 mt-6 rounded-lg shadow relative">
            <div
                title="Edit"
                onClick={() => setShowEditCertificates(true)}
                className="absolute top-4 right-4 justify-between flex cursor-pointer"
              >
                <p
                  className="hover:border border-sec3 rounded-full p-2"
                >
