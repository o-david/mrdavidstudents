...
const ImageEdit = () => {
  ...
  const saveImage = async() =>{
    try{
        await api.put(`${API_URL}/user/update-user`,{profilePicture:newImage} );
        setNewImage("")
        checkAuth();
    }catch(e){
        console.log(e)
      ...
      <div className="sm:w-[45%] w-full flex text-sec3 flex-col items-center gap-10 bg-white py-8 rounded-lg shadow-md relative">
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="flex justify-center gap-4">
            ...
            className="hidden" />
            <button className="border border-red-300 bg-red-300 rounded-lg p-2 transition-all hover:bg-red-500 hover:text-white">
              Delete Image
            </button>
          </div>
        </div>
      </div>
      { newImage && <div className="absolute">
        <button
          onClick={() => setNewImage("")}
